(function (window, undefined) {
	...
	var _speak2 = window.speak2;
	
	// Runs speak2.js in no conflict mode, returning the original 'speak'
	// variable to its owner. Returns a reference to this speak object.
	speak2.noConflict = function () {
		window.speak2 = _speak2;
		return speak2;
	}
	...
	var speakWorker;
	try {
		// https://github.com/yoshi6jp/speak2.js/commit/b85d385024f1e20818aa9e3b272c86aa9fc2ebe6
		speakWorker = new Worker(document.querySelector('script[src$="speakClient2.js"]').getAttribute('src').replace(/speakClient2.js$/, 'speakWorker2.js'));
	} catch (e) {
		console.log('speak2.js warning: no worker support');
	}
	
	speak2.play = function (text, args, onended, onstart) {
		...
		function playSound(streamBuffer) {
			...
			audioContext.decodeAudioData(streamBuffer, function (audioData) {
				...
				if (!isChrome && source.onended !== undefined) {
					...
					if ($('body').hasClass("_")) {
						var duration = audioData.duration + 4000;
						// convert to msecs
		...
		function handleWav(wav) {
			...
			playSound(buffer);
			if (PROFILE) console.log('speak2.js: wav processing took ' + (Date.now() - startTime).toFixed(2) + ' ms');
		}
		...
		if (args && args.noWorker) {
			...
			if (PROFILE) console.log('speak2.js: processing took ' + (Date.now() - startTime).toFixed(2) + ' ms');
			playSound(wav);
		} else {
			// Call the worker, which will return a wav that we then play
			var startTime = Date.now();
			speakWorker.onmessage = function (event) {
				if (PROFILE) console.log('speak2.js: worker processing took ' + (Date.now() - startTime).toFixed(2) + ' ms');
